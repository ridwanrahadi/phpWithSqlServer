...
<h2 align="center">USER AKTIF DENGAN KODE SALES = <?php echo $NamaSales; ?></h2>
 <div class="container">
<div class="jumbotron">
        <canvas id="barchart" style="height: 300px; width: 100%;"></canvas>
    
 <?php 
  $sql1="SELECT MONTH(Tgl) AS Bulan, SUM(Total) AS Total
  FROM TblFak
  ...
  $sql2="SELECT MONTH(Tgl) AS Bulan, SUM(Total) AS Total
  FROM TblFak
  WHERE (KdSales = '$user') AND (YEAR(Tgl) = $tahun)
  GROUP BY MONTH(Tgl) ORDER BY MONTH(Tgl) ASC ";
...
<div class="jumbotron">
  ...
  <table class="table table-striped">
    ...
    <tbody>
      <?php 
  $sql="SELECT MONTH(Tgl) AS Bulan, SUM(Total) AS Total
  FROM TblFak
  WHERE (KdSales = '$user') AND (YEAR(Tgl) = $tahun)
  GROUP BY MONTH(Tgl) ORDER BY MONTH(Tgl) ASC ";
  $stmt  = sqlsrv_query($conn,$sql) or die ("Query  salah:".sqlsrv_error());
