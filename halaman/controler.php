...
if($_GET) {
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			$stmt = sqlsrv_query($conn, $msSql) or die("Query  salah:" . sqlsrv_error());
		}
	...
	if(isset($_POST['btnCek'])){
		...
		$hasil  = sqlsrv_query($conn, $query, $params, $options) or die("Query  salah:" . sqlsrv_error());
		...
		Where KdCust = '".$kdcust."'";
		$myQry  = sqlsrv_query($conn, $query2) or die("Query  salah:" . sqlsrv_error());
...
#
	...
	//	}
		...
		$params = array();
		$options =  array("Scrollable" => SQLSRV_CURSOR_KEYSET);
		$hasil  = sqlsrv_query($conn, $query3, $params, $options) or die("Query  salah:" . sqlsrv_error());
		$rows = sqlsrv_num_rows($hasil);
		...
		$params = array();
		$options =  array("Scrollable" => SQLSRV_CURSOR_KEYSET);
		$hasil  = sqlsrv_query($conn, $query4, $params, $options) or die("Query  salah:" . sqlsrv_error());
		$rows = sqlsrv_num_rows($hasil);
		...
		if (count($pesanError)>=1 ){
			...
			echo "</div>"; 
			   ...
			   sqlsrv_query($conn, $tmpSql) or die("Query  salah:" . sqlsrv_error());
	...
	$query = sqlsrv_query($conn, $sql) or die("Query  salah:" . sqlsrv_error());
	...
	if($kode_faktur){
		...
		$kode = $kode + 1;
		$auto_kode = "SO22-".str_pad($kode, 5, "0",  STR_PAD_LEFT);
	} else {
		$auto_kode = "SO22-00001";
	}
		...
		if (count($pesanError)>=1 ){
			...
			$query  = "SELECT tblCustomer.Saldo,tblCustomer.KreditLimit
						...
						if ($KreditLimit < $GrandTotal) {
							# code...
							$Keterangan2 ="Limit sisa {$KreditLimit}";

						}
			#Simpan Sales ORDER			
			$mySql	= "INSERT INTO salesorder values ('$auto_kode','$tglTransaksi','$jam', '$txtKdCust','$txtNmCust','$txtAlamat','$txtKTP', '$txtnoPO','$txttglPO','$GrandTotal','$txtKeterangan','".$_SESSION['username']."','','$Keterangan2','','','','','','')";
			...
			if($myQry){
				...
				while ($tmpRow = sqlsrv_fetch_array($tmpQry, SQLSRV_FETCH_ASSOC)) {
					...
					sqlsrv_query($conn, $itemSql) or die("Query  salah:" . sqlsrv_error());
				}
				
				# Kosongkan Tmp jika datanya sudah dipindah
				$hapusSql = "DELETE FROM tmp_so WHERE kd_user='".$_SESSION['username']."'";
				sqlsrv_query($conn, $hapusSql) or die("Query  salah:" . sqlsrv_error());
